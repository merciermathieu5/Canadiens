/* Harnais de validation — Canadiens de Montréal (jsdom) */
const fs = require('fs');
...
/* Constantes propres à l'équipe (mise à jour ushl.ca du 1er août 2026).
   masseSousContrat = somme des salaires CT>0 de l'alignement PRO, colonnes
   Salary et CT de TeamRosters.php (MONTREAL). */
const ATTENDU = {
  equipe: 'MONTREAL',
  taille: 24,
  echantillon: {nom: 'Adrian Kempe', salaire: 7500000, ct: 1, ov: 81},
  masseSousContrat: 53000000,
  nbSousContrat: 13
};
...
(async () => {
  ...
  ok(!!S, 'API de test exposée (__MTL__)');
  ...
  const fixtureXtra = [
    ...
    '    [Nikita Kucherov](https://x/#Nikita Kucherov) MONTREAL         RW  82  25  69  94 187  10 1767  81 2.28',
    '    [Alex Tuch](https://x/#Alex Tuch) ANAHEIM         RW  77  27  32  59 183  16 1503 243 2.38',
    '    [* Backup_RW](https://x/#Backup_RW) MONTREAL         LW   3   0   0   0   0   0    0   0 0.00'
  ].join('\n');
  const x = S.parseXtra(fixtureXtra, 'MONTREAL');
  egal(x.length, 2, 'Deux patineurs MONTREAL extraits (Anaheim exclu)');
  egal(x[0].hits, 81, 'MEÉ de Kucherov depuis XtraStats');
  egal(x[0].mp, 1767, 'Minutes de Kucherov depuis XtraStats');
  ...
  console.log('— Parseur de la page «Finances» (contrats à jour, dont les 0 an)');
  {
    const fxFin = `<html><body>
      <p>MONTREAL 0-0-0 (10e Eastern)</p>
      <table><tr><th>JOUEUR</th><th>A</th><th>CONTRAT</th></tr>
        <tr><td>Adrian Kempe</td><td>3</td><td>7 500 000 $</td></tr>
        <tr><td>Victor Hedman</td><td>0</td><td>8 450 000 $</td></tr>
      </table>
      <table><tr><th>JOUEUR</th><th>A</th><th>CONTRAT</th></tr>
    ...
    egal(contrats.get(S.normaliserNom('Victor Hedman')).ct, 0, 'Hedman : 0 année de contrat (la donnée qui manquait!)');
    egal(contrats.get(S.normaliserNom('Victor Hedman')).salaire, 8450000, 'Salaire de Hedman lu');
    egal(contrats.get(S.normaliserNom('Victor Hedman')).clubEcole, false, 'Première table = pros');
    egal(contrats.get(S.normaliserNom('Jeune Espoir')).clubEcole, true, 'Table suivante = club-école');
    egal(contrats.get(S.normaliserNom('Adrian Kempe')).ct, 3, 'CT de Kempe lu à 3');
  }
  ...
  const vKe = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Adrian Kempe'), 'UFA', 1);
  egal(vKe.code, 'surpaye', 'Adrian Kempe (OV 81, 30 ans, 7,5 M, UFA) → Surpayé');
  egal(vKe.minEff, 6250000, 'Charte Kempe : 6 250 000 $');
  egal(vKe.ecart, 1250000, 'Écart Kempe : +1 250 000 $');
  const vAll = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Nolan Allan'), 'RFA', 1);
  egal(vAll.code, 'aubaine', 'Nolan Allan (OV 78, 23 ans, 900 k, RFA) → Aubaine salariale');
  egal(vAll.ecart, 1350000, 'Écart Allan : −1 350 000 $ sous la charte (2,25 M)');
  ...
  const vPor = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Erik Portillo'), 'UFA', 1);
  egal(vPor.minEff, 2250000, 'Erik Portillo (G, OV 79, 26 ans) évalué à l\'échelon 78 : 2 250 000 $');
  egal(vPor.code, 'aubaine', 'Erik Portillo à 750 k → Aubaine salariale');
  ...
  egal(S.verdictCharte({nom:'Backup_C', po:'C', ov:55, age:25, salaire:0, ct:0, backup:true}, 'RFA', 1), null,
    'Les joueurs de remplacement (backup) sont inanalysables (fixture : aucun backup dans l\'alignement MONTREAL)');
  ...
  selJoueur.value = 'Adrian Kempe';
  selJoueur.dispatchEvent(new W.Event('change'));
  ok(doc.getElementById('csVerdict').classList.contains('surpaye'), 'Kempe sélectionné : badge Surpayé');
  ...
  selJoueur.value = 'Erik Portillo';
  selJoueur.dispatchEvent(new W.Event('change'));
  ok(doc.getElementById('csVerdict').classList.contains('aubaine'), 'Erik Portillo : badge Aubaine salariale');
  ok(doc.querySelector('#csFiche').textContent.includes('78'), 'Échelon gardien (OV −1 → 78) affiché dans la fiche');
